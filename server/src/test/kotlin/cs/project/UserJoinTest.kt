...
import cs.shared.project.model.NewJoinerMessage
...
class UserJoinTest {
    ...
    @Test
    fun userJoinedInBroadcastToSubscribersOfWhiteboard() {
        val numClients = 2
                    ...
                    override fun afterConnected(session: StompSession, connectedHeaders: StompHeaders) {
                        ...
                        session.subscribe("/user/topic/join", object : StompFrameHandler {
                            override fun handleFrame(headers: StompHeaders, payload: Any?) {
                                ...
                                session.subscribe(
                                    "/topic/new-joiner/" + whiteboard.id,
                                    object : StompFrameHandler {
                                        override fun handleFrame(headers: StompHeaders, payload: Any?) {
                                            val msg =
                                                "New joiner message received: ${(payload as NewJoinerMessage).author}"
                                            clientsMessages[i].offer(msg)
                                        }
        ...
        // THEN: check if client 1 received a new-joiner message saying that client 2 joined
        val response = client1Messages.poll(2, TimeUnit.SECONDS)
        assert(response == "New joiner message received: ${client2Session!!.sessionId}")
    }
}